...
export const SeatButton = memo(function SeatButton({
  ...
  const priceText = price > 0
    ? `, ${priceFormatter ? priceFormatter(price) : `$${price}`}`
    : "";
  const ariaLabel = `${isBerth ? "Berth" : "Seat"} ${label}, ${effectiveStatus}${priceText}`;

  const iconCls = cn("absolute inset-0 w-full h-full", isBerth ? "p-0.5" : "p-1", ICON_STYLES[effectiveStatus]);
  ...
  const button = (
    ...
    >
      ...
      {effectiveStatus === "selected" && (
        <svg viewBox="0 0 16 16" className="absolute top-0.5 right-0.5 size-3 text-white" aria-hidden="true">
      ...
      {effectiveStatus === "booked" && (
        <svg viewBox="0 0 16 16" className="absolute top-0.5 right-0.5 size-3 text-white/80" aria-hidden="true">
          <path fill="currentColor" d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM2 13c0-2.21 2.69-4 6-4s6 1.79 6 4v1H2v-1Z" />
        </svg>
      )}
      {effectiveStatus === "blocked" && (
        <svg viewBox="0 0 16 16" className="absolute top-0.5 right-0.5 size-3 text-white/80" aria-hidden="true">
          <path fill="currentColor" d="M8 1a4 4 0 0 0-4 4v2H3a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V8a1 1 0 0 0-1-1h-1V5a4 4 0 0 0-4-4Zm2 6H6V5a2 2 0 1 1 4 0v2Z" />
        </svg>
      )}
    </button>
  );
